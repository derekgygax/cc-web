...
$("body").on("click", "#recommendedSingle .recommended_approve", function(e) {
	...
	couple.approveProfile({"approve": datavalue}, function(res){
//		console.log(res)
	});
...
//			console.log("Redirecting to Couple Profile: /profile?couple="+nextRecommenedCouple);
			window.location.href="/profile?couple="+nextRecommenedCouple;
			couple.approveProfile({"approve": datavalue}, function(res){
//				console.log(res)
			});
		} else {
...
//				console.log(res)
			...
			window.location.href="/recommended_couples";
		}
		...
		/* Basically move the card to the right Tab after we perform an action on it */
		/* make sure to change the text and data-value's to prevent weirdness */
		if(tab == 'recommendedCouples') {

			if(datavalue == 'approve') {
				$("#approvedCouples").append(el);
				// Remove the approve button
				$(el).find(".recommended_approve[data-value='approve']").parent().remove();
			}
			if(datavalue == 'decline') {
				$("#declinedCouples").append(el);
				// Remove the decline button
				$(el).find(".recommended_approve[data-value='decline']").parent().remove();
			}
		}
		if(tab == 'approvedCouples') {
			if(datavalue == 'decline') {
				$("#declinedCouples").append(el);
				// Change the decline button to an approve button
				var ApproveBtn = $(el).find(".recommended_approve[data-value='decline']");
		...
		if(tab == 'declinedCouples') {
			if(datavalue == 'approve') {
				$("#approvedCouples").append(el);
				// Change the approve button to a decline button
				var declineBtn = $(el).find(".recommended_approve[data-value='approve']");
		...
		let launchNewMessageBlock = function(matchInfo){
			...
			var futureRoomId = matchInfo.matchId;
		...
		couple.approveProfile({"approve": datavalue}, function(rText){
			rText = JSON.parse(rText);
			if (rText.isMatch == true){
				launchNewMessageBlock(rText);
			}
		});
