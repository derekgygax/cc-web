$(document).ready(function(){
	...
	let resetRequirements = function(localServlet){
			...
			if(localServlet =='/cc-web/profile/update') {
			...
			} else if(localServlet =='/cc-web/couple/update'){
				...
				var story = $("#updateCoupleForm").find("textarea[name='story']").val();
			...
			$.ajax({
				type: "GET",
				url: "/cc-web/requirements?onlycontent=true",
...
//					console.log(requirementsNode);
	...
	let changeRequirementProgress = function() {
		...
		if($("body").find("#updatePartnerForm").length) {
		...
		if($("body").find("#updateCoupleForm").length) {
			var stage = $("#requirementsProgressBar").children().eq(3);
			$("#requirementsProgressBar li.active").removeClass("active");
			$(stage).addClass("active");
			return;
		}
		
		if($("body").find(".coupleUpForm").length){
	...
	let Requirements = {
			'servlets': [
				'/cc-web/create_couple',
				'/cc-web/images',
				'/cc-web/couple/update',
				'/cc-web/profile/update',
				'/cc-web/survey',
			],
			pullTrigger: true,
			active: false,
	...
	$(document).on("ajaxFinishedLoading", function(event, data){
		if (Requirements.active && Requirements.servlets.indexOf(data.localServlet) > -1){
			...
			if (data.localServlet == "/cc-web/survey"){
				let reset = true;
