(function(){
	...
	let GroupOfSockets = {
		openSockets: function(matches, homePartner, awayPartner, urlPath){
			this.sockets = {};
			...
			let makeSocketConnection = function(match){
				...
				// Once you can establish a connection do this!

				if(!match.matchId) {
					match.matchId = homePartner.id+"-" + awayPartner.id + "-"+match.awayCoupleId;
				}

				socket.setup(match, homePartner, awayPartner);
	...
	let NotificationSocket = {
		setup: function(match, homePartner, awayPartner){
...
//			this.messages_saved
	...
	$(document).on("send_future_chat_message", function(event, eventData){
		...
		socket = Object.create(ChatSocket);
		socket.setup(eventData.match.match, eventData.homePartner, eventData.awayPartner);

		if (eventData.message){
			socket.sendMessage(eventData.message);
