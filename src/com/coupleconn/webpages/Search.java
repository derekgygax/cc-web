...
//			System.out.println(couplesStr);
			...
			ArrayList<JsonMatch> searchResults = new ArrayList<JsonMatch>();
			
			for (Couple couple : couples) {
				String coupleId = couple.coupleId;
				HashMap<String, String> matchInfoStr = new Api("matches", true, request)
						.get(new String[]{coupleId}, new HashMap<String, String>());
				
				if(!matchInfoStr.get("status").equals("200")) {
					JsonMatch matchInfo = new JsonMatch();
					matchInfo.couple = couple;
					matchInfo.isMatch = false;
					matchInfo.approve = "none";
					matchInfo.partnerApprove = "none";
					matchInfo.coupleApprove = "none";

					searchResults.add(matchInfo);
					continue;
				}
				
				JsonMatch matchInfo = om.readValue(matchInfoStr.get("text"), new TypeReference<JsonMatch>(){});
				matchInfo.couple = couple;
